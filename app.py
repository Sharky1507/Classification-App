...
def getClassifier(classifier):
    if classifier == 'SVM':
        ...
        learning_rate = st.sidebar.slider('Learning Rate', 0.01, 0.5, step=0.01)
        n_estimators = st.sidebar.slider('Number of Estimators', 50, 500, step=50)
        model = GradientBoostingClassifier(learning_rate=learning_rate, n_estimators=n_estimators, random_state=1)
...
classifier = sideBar.selectbox('Which Classifier do you want to use?',('SVM' , 'KNN' , 'Random Forest', 'Decision Tree', 'Gradient Boosting', 'Naive Bayes'))
